...
def test(n,qv,gt,hweight=1,opts={}):
...
def test2(gt,psize=11):
	...
	for n in range(2,7):
		...
		for _ in range(11):
			...
			print("batch",_)
			# split cases
			qv=[bbb.random().copy() for _ in range((n+_)*2)]
			print("qv done")
			# clear results
			...
			for rud in range(3):
				...
				for i in range(len(newpop)):
					...
					print("P",i)
					res=test(n,qv,gt,hweight=p[1].w,opts={"steps":maxstep})
					p[0]=res
					print(res)
				print("score done")
				newpop.sort()

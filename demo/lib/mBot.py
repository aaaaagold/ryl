...
import glob,struct
from multiprocessing import Process,Manager,Array,freeze_support
import threading
import hid
...
class mBot():
	...
	def doMove(self,leftSpeed,rightSpeed):
		if  leftSpeed<0:  leftSpeed=int( leftSpeed) + int( leftSpeed//8)
		if rightSpeed<0: rightSpeed=int(rightSpeed) + int(rightSpeed//8)
		self.__writePackage(bytearray([0xff,0x55,0x7,0x0,0x2,0x5]+self.short2bytes(-leftSpeed)+self.short2bytes(rightSpeed)))

	def doMoveB(self,leftSpeed,rightSpeed,Bias=[1,1]):
		return self.doMove(leftSpeed*Bias[0],rightSpeed*Bias[1])
